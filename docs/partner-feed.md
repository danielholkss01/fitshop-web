...
This is a manual first step. Larger feeds should move to scheduled ingestion and a database with indexed candidate retrieval and page cursors rather than committing every refresh to Git. The current generator ranks every combination in memory, which suits the demo catalogue but would be expensive for a large feed. The site sends shoppers to the retailer for purchase and does not take payments.
